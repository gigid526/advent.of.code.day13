...
foreach ($lines as $line) {
    for ($x = 0; $x < strlen($line); $x++) {
        if (array_key_exists($line{$x}, $cartDecoder)) {
			...
			array_push($carts, [$x, $y, $direction, 0, 0]); // last params means `to remove`
...
while (count($carts) > 1) {
	...
	foreach (array_keys($carts) as $idx) {
		if ($carts[$idx][4]) {
			unset($carts[$idx]);
		}
	}
	$carts = array_slice($carts, 0);
}
echo 'The last cart: ' . implode(', ', $carts[0]) . PHP_EOL;
